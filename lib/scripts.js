addEventListener("DOMContentLoaded", (event) => {
 ...
 gsap.utils.toArray(".gsap-cursor").forEach((el) => {
  gsap.set(el, {xPercent: -50, yPercent: -50});
 
    let xTo = gsap.quickTo(el, "x", {duration: 0.6, ease: "power3"}),
  ...
  aLinks.forEach((el, i) => {
   el.addEventListener("mouseenter", e => {
    document.querySelector(el).classList.add("big-cursor");
   });
 
   el.addEventListener("mouseleave", e => {
    document.querySelector(el).classList.remove("big-cursor");
   });
  });
